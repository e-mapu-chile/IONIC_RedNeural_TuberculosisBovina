...
export class MapasPage {
  ...
  async pintarPuntSsa(clas, lat, longt, rup, element) {
    //console.log(clas)
    var uno = Leaflet.icon({
      iconUrl: '../assets/img/' + clas + '.png',
      shadowUrl: '../assets/img/' + clas + '.png',
    ...
    markPoint.bindPopup('<p>RUP: ' + rup + '</p>', {
      ...
      this.semanaEpi = "";
      this.protocoloId = "";
      if (element.NombreEstablecimiento != undefined)
        this.nombreEstablecimiento = element.NombreEstablecimiento
      if (element.ProtocoloId != undefined)
        this.protocoloId = element.ProtocoloId
      if (element.Positivos != undefined)
        this.cantidadPositivos = element.Positivos
      if (element.Negativos != undefined)
        this.cantidadNegativos = element.Negativos
      this.semanaEpi = "";
      if (element.anio != undefined)
        this.anioEpi = element.anio;
      if (element.anio == 2022) {
        this.semanaEpi = element.semana22;
      }
      if (element.anio == 2023) {
        this.semanaEpi = element.semana23;
      }
  ...
  async jsonMapaSSA(jsonPointResultadosSsa) {
    console.log(JSON.stringify(jsonPointResultadosSsa));
    ...
    if (jsonPointResultadosSsa.length > 0) {
      const xL = await this.cargando("Espere, insertando resultados");
    }
    for await (const element of jsonPointResultadosSsa) {
      // if (this.logicaFiltro(element)) {
      // console.log("fi=> si")
      const dat = dataJO.filter(d => d.Rup == element.RupTomaMuestra)
      var dataA = dat || [];
      let icono = "911_911"
      if (dataA.length > 0) {
        icono = dataA[0].RubroId + "_" + dataA[0].RubroId;
        // console.log(icono);
      }
      if (element.Positivos > 0) {
        icono += "_1";
        console.log(icono);
      }
      else {
      ...
      await this.pintarPuntSsa(icono, element.Latitud, element.Longitud, element.RupTomaMuestra, element);
      // } else {
  ...
  async jsonMapa(jsonPointEstab) {
    console.log(JSON.stringify(jsonPointEstab));
    //await this.limpiarAll();
    if (jsonPointEstab.length > 0) {
      const xL = await this.cargando("Espere, insertando predios categorizados");
    }
    for await (const element of jsonPointEstab) {
      // if (this.logicaFiltro(element)) {
      // console.log("fi=> si")
      let icono = element.RubroId + "_" + element.RubroId;
  ...
  flagU3 = 0;
  async irDatosSSA() {
    const dataJ = await this.storage.get('resultadosIA')
    var dataJO = dataJ || [];
    if (dataJO.length < 1) {
      const xL = await this.cargando("Espere");
      this.getResultadosIA().subscribe((resp) => {
        let ss = JSON.stringify(resp, null, 4);
        console.log("respuesta API resultados IA= " + ss);

        // this.crearDataSetMovIn(resp, this.region);
        let dataJ = JSON.parse(ss);

        this.storage.set('resultadosIA', dataJ)
        this.loading.dismiss();
        this.limpiarInformacionRup();


        this.pintarIAResultadoSSA();
      ...
      await this.pintarIAResultadoSSA();
  ...
  async irDatos() {
    ...
    if (dataJO.length < 1) {
      const xL = await this.cargando("Espere");
      this.getEstablecimientos().subscribe((resp) => {
        let ss = JSON.stringify(resp, null, 4);
      ...
      await this.pintarIAEstabSipec();

    }


  }
  async pintarIAResultadoSSA() {
    const dataJ = await this.storage.get('resultadosIA')

    var dataJO = dataJ || [];
    var filtroRegion = dataJO.filter(f => f.RegionId == this.regionSelect);
    var regionJson = filtroRegion || [];
    this.jsonMapaSSA(regionJson);
  }

  async pintarIAEstabSipec() {
    const dataJ = await this.storage.get('establecimientosAves')

    var dataJO = dataJ || [];
    var filtroRegion = dataJO.filter(f => f.RegionId == this.regionSelect);
    var regionJson = filtroRegion || [];
    this.jsonMapa(regionJson);
  }
  flagU = 0;
  ...
  getEstablecimientos() {
    ...
    return this.http.get(
      'http://127.0.0.1:5000/api/getPrediosGrupoEspecie?grupoEspecie=13',
  ...
  getResultadosIA() {
    const headers = new HttpHeaders().set("Authorization", "").set('Content-Type', 'text/plain; charset=utf-8');
    return this.http.get(
      'http://127.0.0.1:5000/api/getResultadosIA',
